...
function installContext(root, scripts) {
  const nodes = new Map();
  let edges = 0;
  let truncated = false;
  ...
  return {
    add(file, code, mask, indicators) {
      if (nodes.size >= MAX_FILES || edges >= MAX_EDGES) { truncated = true; return; }
      const references = importReferences(code, mask);
    ...
    finish(sourceCoverageComplete) {
      return { schemaVersion: 1, analysis: 'literal-import-graph', sourceCoverageComplete, truncated,
...
function installLines(context) {
  ...
  const lines = ['Potential paths from installation scripts (literal imports only):'];
  ...
  lines.push('Syntactic reachability is not proof of execution or data transfer. Aliases, conditions and runtime resolution are not evaluated.');
  return lines;
}
