...
const fs = require('fs');
const { astImports } = require('./ast-imports');
...
function installContext(root, scripts, parser = null, packageType = 'commonjs') {
  ...
  return {
    add(file, code, mask, indicators, original = code) {
      if (nodes.size >= MAX_FILES || edges >= MAX_EDGES) { truncated = true; return; }
      let references;
      if (parser) {
        let result;
        try { result = astImports(parser, original, file, sourceType(file.replace(/\\/g, '/'))); }
        catch (_) { result = { parsed: false, references: [{ line: 1, reason: 'ast-analysis-error' }] }; }
