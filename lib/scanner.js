  ...
  for (let i = 0; i < codeLines.length; i++) {
    ...
    for (const cat of CATEGORIES) {
      for (const pattern of cat.patterns) {
        const hit = pattern.exec(codeLine);
        if (hit) {
          caps[cat.key].present = true;
          addEvidence(caps[cat.key].evidence, relPath, lineNo, originalLine, pattern, 5, hit.index);
          break;
        }
      }
    }
...
function scanScriptCommand(scriptKey, command, caps) {
  ...
  for (const cat of CATEGORIES) {
    for (const pattern of cat.patterns) {
      ...
      if (hit) {
        caps[cat.key].present = true;
        addEvidence(caps[cat.key].evidence, where, 1, command, pattern, 5, hit.index);
        break;
      }
    }
  }
