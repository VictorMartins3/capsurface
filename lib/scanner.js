...
function matchRule(rule, text) {
  ...
  if (rule.context && !rule.context.test(text)) return null;
  return rule.match.exec(text);
}
...
function addEvidence(list, file, lineNo, line, pattern, max = 5, matchIndex = -1) {
    ...
    for (const cat of CATEGORIES) {
      for (const rule of cat.patterns) {
        ...
        if (hit) {
          ...
          addEvidence(caps[cat.key].evidence, relPath, lineNo, originalLine, ruleSource(rule), 5, hit.index);
          break;
        }
      }
    }
...
function scanScriptCommand(scriptKey, command, caps) {
  ...
  for (const cat of CATEGORIES) {
    for (const rule of cat.patterns) {
      ...
      if (hit) {
        caps[cat.key].present = true;
        addEvidence(caps[cat.key].evidence, where, 1, command, ruleSource(rule), 5, hit.index);
        break;
      }
    }
  }
  ...
  while ((m = ENV_VAR_PATTERN.exec(command)) !== null) {
    const name = m[1] || m[2];
    if (name && caps.env.vars.length < 40 && !caps.env.vars.includes(name)) caps.env.vars.push(name);
  }
}
