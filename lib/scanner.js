...
const SKIP_DIRS = new Set(['node_modules', '.git', 'test', 'tests', '__tests__', 'docs', 'examples']);
...
const BUILD_ARTIFACT_PATH = /(^|[\\/])(dist|umd|build|cjs|esm|lib-esm|vendor)([\\/]|$)|[.\-]min\.[jt]sx?$|\.bundle\.[jt]sx?$/i;
...
function addEvidence(list, file, lineNo, line, max = 5) {
  if (list.length >= max) return;
  const snippet = line.trim().slice(0, 200);
  list.push({ file, line: lineNo, snippet });
}
...
function scanFileContent(relPath, content, caps) {
  ...
  for (let i = 0; i < codeLines.length; i++) {
    ...
    if (!isBuildArtifact && codeLine.length > LONG_LINE_THRESHOLD) {
      caps.obfuscationSignal.present = true;
      addEvidence(caps.obfuscationSignal.evidence, relPath, lineNo, `<line of length ${codeLine.length}>`, 3);
    }
    ...
    for (const cat of CATEGORIES) {
      for (const pattern of cat.patterns) {
        if (pattern.test(codeLine)) {
          ...
          addEvidence(caps[cat.key].evidence, relPath, lineNo, originalLine);
          break;
        }
      }
    }
...
function computeRisk(caps) {
  ...
  const hasEnv = caps.env.present;
  const hasExec = caps.exec.present;

  if (hasInstallTriggering && hasNetwork && (hasSensitive || hasEnv)) {
    score += 10;
    flags.push(
  ...
  } else if (hasBuildTimeOnly && hasNetwork && (hasSensitive || hasEnv)) {
    flags.push(
