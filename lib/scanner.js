...
// How far from a match its context has to be. A minified bundle is one
// enormous line, so a context scoped to the whole line is satisfied by
// anything anywhere in the file: @claudiolabs/claudin was CRITICAL because
// its bundle carries '~/.ssh/config' in a CLI help string and, tens of
// kilobytes away on the same line, calls something ending in `open(`. On an
// ordinary source line this window covers the whole line and changes
// nothing.
...
function readPackageJson(dir) {
  ...
  try {
    // npm strips a leading byte-order mark and so does every real consumer,
    // so a BOM-prefixed package.json is a valid published package. 6 of
    // 20,039 have one, and every one was being reported as malformed.
...
function computeRisk(caps) {
  ...
  // The worm pattern needs credentials that belong to the developer's
  // environment rather than to the package: an npm or GitHub token, an SSH
  // key, AWS keys, ~/.npmrc. Those are what let a compromised package
  // publish itself onward, which is what makes it self-propagating.
  //
  // A credential-shaped env var is not that. Across 20,039 published
  // packages this flag fired 37 times and 29 of those were a tool reading
  // its own service credential: figma-image-exporter runs a postinstall,
  // talks to api.figma.com and reads FIGMA_TOKEN, and was being told it
  // matched Shai-Hulud. Those 29 are still reported, one severity down and
  // described as what they are. The 8 that remain all genuinely reach for
...
function scanPackageDir(dir) {
  ...
  if (pkgJson && pkgJson.scripts) {
    for (const key of LIFECYCLE_SCRIPT_KEYS) {
      ...
      // `prepare` does not run for a registry install, and crediting the
      // package for what it does would contradict scoring it lower for the
      // same reason: glob's prepare runs `tshy && bash scripts/build.sh`,
      // which made a routine glob 10 to 13 upgrade fail the gate on process
      // execution the consumer never sees. The command is still recorded,
      // and diff.js already reports a changed prepare without gating on it.
