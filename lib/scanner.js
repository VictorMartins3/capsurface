...
function scanFileContent(relPath, content, caps, collect, context, install) {
  const longLines = [];
  ...
  if (install) install.add(relPath, codeOnly, blankErasedSyntax(blankComments(content, true), relPath), context.indicators());
}
...
function scanPackageDir(dir) {
  ...
  const install = Object.keys(installScripts).length ? installContext(dir, installScripts) : null;
  ...
  for (const file of files) {
    ...
    scanFileContent(relPath, source.toString('utf8'), caps, collect, local, install);
    context.add(local);
  }
  ...
  return {
    schemaVersion: 8,
    installContext: install ? install.finish(coverage.complete) : { schemaVersion: 1, analysis: 'literal-import-graph', sourceCoverageComplete: coverage.complete, truncated: false, hooks: [] },
