...
function blankComments(content) {
  // Only comment characters ever differ from the input, so record where the
  // comments are and splice blanks over those ranges, rather than rebuilding
  // the file one character at a time.
  const commentRanges = [];
  let commentStart = -1;
  const NORMAL = 0;
...
function scanFileContent(relPath, content, caps, collect) {
  const longLines = [];
...
function scanPackageDir(dir) {
  ...
  for (const file of files) {
    ...
    scanFileContent(relPath, source.toString('utf8'), caps, collect);
  }
