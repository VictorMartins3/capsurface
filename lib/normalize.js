...
// Bindings only ever substitute `require(name)`, so a file with no such call
// needs none. Skipping those files avoids the collection pass entirely.
...
// Each fold is gated on a substring test first. Almost no line contains any
// of these, so the regex only runs where it can match.
...
/**
 * Normalize one line. Returns the line unchanged when nothing folded, which
