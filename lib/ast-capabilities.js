...
const BUILTINS = new Map([
  ...
  ...['http', 'https', 'http2', 'net', 'dgram', 'tls', 'dns', 'dns/promises'].map((name) => [name, 'network']),
...
function recordAstCapabilities(file, content, references, caps, context, operations = []) {
  ...
  let line = 1;
  let start = 0;
  for (const reference of [...references, ...operations.flatMap((op) => op.parent ? [{ ...op, category: op.parent }, op] : [op])].sort((a, b) => a.line - b.line)) {
    ...
    if (cap.evidence.length < 5 && !cap.evidence.some((item) => item.file === file && item.line === reference.line)) {
      cap.evidence.push({ file, line: reference.line, snippet, pattern: reference.rule || (reference.method ? `node:child_process.${reference.method}` : 'ast-module-acquisition'),
