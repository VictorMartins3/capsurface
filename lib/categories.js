...
const CATEGORIES = [
  ...
  },
  {
    ...
    severity: 5,
    // Credential env vars must appear as an actual read, not a mention.
    // Across 3,440 real packages the bare-name rules matched help text
    // ("Set the GITHUB_TOKEN environment variable"), assertions in shipped
    // tests, and string literals in config lists. Requiring process.env
    // access keeps the real reads, nx and install-artifact-from-github both
    // do `process.env.GITHUB_TOKEN || process.env.GH_TOKEN`, and drops the
    // prose. File-path rules stay as plain matches: a package naming
    // ~/.ssh/id_rsa is worth a look regardless of the surrounding syntax.
    patterns: [
      /\.npmrc\b/,
      /\.ssh\//,
      /\bid_rsa\b/,
