...
const CATEGORIES = [
  ...
  },
  {
    ...
    patterns: [
      /\b(?:require|import)\s*\(\s*['"`](node:)?(https?|http2|net|dgram|tls|dns)['"`]\s*\)/,
      ...
      /\bnew\s+WebSocket\s*\(/,
      /\b(?:require|import)\s*\(\s*['"`](axios|node-fetch|undici|got|superagent|request)['"`]\s*\)/,
  ...
  },
  {
    ...
    patterns: [
      ...
      /\bvm\.(runIn|Script)\b/,
      /\b(?:require|import)\s*\(\s*['"`](node:)?vm['"`]\s*\)/,
      /\bfrom\s+['"](node:)?vm['"]/,
    ],
  },
  {
    ...
    patterns: [
      /\b(?:require|import)\s*\(\s*['"`](ffi-napi|bindings|node-gyp-build|node-gyp-build-optional-packages)['"`]\s*\)/,
