...
const CATEGORIES = [
  ...
  },
  {
    ...
    patterns: [
      /\brequire\s*\(\s*['"`](node:)?(https?|http2|net|dgram|tls|dns)['"`]\s*\)/,
      ...
      /\bnew\s+WebSocket\s*\(/,
      /\brequire\s*\(\s*['"`](axios|node-fetch|undici|got|superagent|request)['"`]\s*\)/,
  ...
  },
  {
    ...
    patterns: [
      /\brequire\s*\(\s*['"`](ffi-napi|bindings|node-gyp-build|node-gyp-build-optional-packages)['"`]\s*\)/,
...
// folded everything it can. The module being loaded is then unknown, and so
// is every capability that comes with it: a package doing
// `require(deobfuscate(payload))` otherwise reports nothing at all, which
// reads exactly like an inert package.
//
// Excludes a property named require (`mod.require(x)`) and webpack's
// `__webpack_require__`, neither of which is the module loader.
// A path anchored inside the package resolves to a file the walk already
// read, so its capabilities are in the manifest whether or not the specifier
// was a literal. `require(path.join(__dirname, name))` is the common shape
// and it is not a loss of visibility.
...
// documented behaviour `prepare` and `prepublish` run only for local
