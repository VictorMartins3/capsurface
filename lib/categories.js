...
// Matched within a window around the path, not across the whole line; see
// CONTEXT_WINDOW in scanner.js for why that matters on minified output.
const PATH_ACCESS =
...
const CATEGORIES = [
  {
    ...
    severity: 2,
    // The `node:` prefix is the documented modern spelling and it was not
    // matched here, only in the network category. `require('node:fs')` and
    // `import fs from 'node:fs/promises'` were invisible, which is how a
    // glob release read as having no filesystem access at all.
    patterns: [
  ...
  },
  {
    ...
    // one, because `$` is not a word character. puppeteer-core's `$eval` and
    // `$$eval` are its DOM query helpers, `redis.eval(...)` runs a Lua script
    // on the server, and cuvp-lowcode-code-generator defines its own
    // `__$$eval`. 71 packages of 20,039 had nothing but one of those. The
    // explicit global forms are real and kept.
    patterns: [
      /(?<![.$\w])eval\s*\(/,
      /\b(?:globalThis|window|self|global)\.eval\s*\(/,
      /\bnew\s+Function\s*\((?!\s*(['"`])return (this|globalThis)\1\s*\))/,
      /\bvm\.(runIn|Script)\b/,
      // Acquiring vm counts even when the call site is a bare destructured
      // name, for the same reason the categories above key on the import.
      /\brequire\(\s*['"](node:)?vm['"]\s*\)/,
      /\bfrom\s+['"](node:)?vm['"]/,
    ],
  },
  {
...
// A URL literal ends at whitespace, a quote, a paren, a backslash (which in
// source is the start of an escape, not part of the URL), an angle bracket
// (markdown and HTML link syntax) or a comma, which is how a config string
// joins two of them: "https://a.example/v1,https://b.example/v2" was being
// recorded as a single endpoint. A comma is legal inside a path and rare
// there, and the host, which is what a reviewer reads, survives either way.
const URL_PATTERN = /https?:\/\/[a-zA-Z0-9\-._~%]+(?::[0-9]+)?[^\s'"`)<>,\\]*/g;

// Sentence punctuation glued to a URL in a comment or an error message.
const ENDPOINT_TRAILING_JUNK = /[.,;:!?]+$/;

// A host has to look like one. `https://.` matched the pattern above and was
// recorded as an endpoint. Names need a dot, and localhost, IPv4 and
// bracketed IPv6 are the three that do not.
const ENDPOINT_HOST =
...
// shell runs it through its shebang. 502 of the 4,502 packages in a registry-wide
// sample that ship an executable point `bin` at a file the extension filter
// never reads, 11.2% of them, and that file is the code a consumer
// runs directly.
...
const INSTALL_COMMAND_RULES = [
  ...
  // Anything piped into a shell is the download-and-run shape. The pipe has
  // to be a real one: `node postinstall.js || bash install.sh` was matching
  // on the second character of `||`, and it was the only match this rule
  // had across 20,039 packages.
  { key: 'exec', pattern: /(?<!\|)\|(?!\|)\s*(sudo\s+)?(ba|z|k|da)?sh\b/ },
  // Handing a script to a shell is process execution however it is reached,
  // which is what that mismatch was accidentally covering: 7 packages run
  // `bash something.sh` from a lifecycle script, onibrowser's postinstall
  // among them.
