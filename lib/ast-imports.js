...
function astImports(parser, code, file, sourceType) {
  ...
  for (const node of nodes) {
    ...
    if (target && target.type === 'MemberExpression') {
      ...
      if (object && ['module', 'process-module'].includes(object.kind)) return failure('ast-module-mutation', node.loc.start.line);
    }
    ...
    for (const argument of exposed) {
      ...
      if (object && ['module', 'process-module'].includes(object.kind)) return failure('ast-module-escape', node.loc.start.line);
    }
  }
  const references = [];
  const operations = [];
...
function createAstAnalyzer(root, parser, packageType = 'commonjs') {
  ...
  return (file, code) => {
    try { return astImports(parser, code, file, sourceType(file.replace(/\\/g, '/'))); }
