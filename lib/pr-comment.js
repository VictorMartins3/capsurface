...
async function publishComment({ event, eventName, repository, key = 'default', report, gate, incomplete, request }) {
  ...
  if (eventName !== 'pull_request' || !pr || pr.head.repo.full_name !== repository || pr.base.repo.full_name !== repository) return 'skipped-event';
  ...
  const signature = `<!-- capsurface-result:${hash} -->`;
