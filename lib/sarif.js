...
function renderSarif(report) {
  const results = report.entries.map((entry) => {
    const ruleId = entry.capabilities.includes('analysisIncomplete') || (entry.coverage && !entry.coverage.complete)
      ? 'capsurface/incomplete-analysis' : entry.match.kind === 'ambiguous' ? 'capsurface/ambiguous-baseline'
        : entry.newPackage ? 'capsurface/new-package' : entry.escalated ? 'capsurface/escalation' : 'capsurface/change';
